        ...
        {others.map((student, index) => {
          ...
          const animClass = matchedConfig
            ? (matchedConfig.animationType === 'float' ? 'animate-avatar-float' :
               ...
               matchedConfig.animationType === 'pulse' ? 'animate-avatar-pulse-subtle' :
               matchedConfig.animationType === 'spin' ? 'animate-avatar-spin-slow' : '')
            : '';
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <img 
                      src={student.avatar} 
                      alt={student.name} 
                      className={`w-11 h-11 rounded-full bg-white border border-slate-100 shadow-sm ${animClass}`} 
                    />
